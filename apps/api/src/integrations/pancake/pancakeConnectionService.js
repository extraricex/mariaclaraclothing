function publicStatus(config, stored = {}) {
  ...
  return {
    ...
    shopId: String(config.shopId || stored.shopId || ''),
    warehouseId: String(config.warehouseId || stored.warehouseId || ''),
    orderSourceId: String(config.orderSourceId || stored.orderSourceId || ''),
    currencyCode: stored.currencyCode || '',
    currencyStatus: stored.currencyStatus || 'unknown',
    priceUnitStatus: stored.priceUnitStatus || 'unknown',
    shopLocked: Boolean(config.shopId || stored.shopLocked),
    warehouseLocked: Boolean(config.warehouseId || stored.warehouseLocked),
    orderSourceLocked: Boolean(config.orderSourceId || stored.orderSourceLocked),
